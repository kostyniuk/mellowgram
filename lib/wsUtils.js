...
module.exports = {
  ...
  sendMessageToDb: async ({ roomId, senderId, context }) => {
    try {
      const query = `INSERT INTO messages (room_id, sender_id, context) VALUES ($1, $2, $3) RETURNING message_id, send_at`;
  ...
  setRead: async ({roomId, senderId}) => {
    //UPDATE messages SET is_read=true WHERE room_id=6 AND sender_id!=8;
    try {
